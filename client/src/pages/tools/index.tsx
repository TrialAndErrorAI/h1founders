          ...
          {tools.map((tool) => {
            ...
            return (
              <Link
                ...
                className={`
                  block bg-background-secondary border rounded-lg p-6 transition-all duration-200
                  ${isLive
                    ? 'border-accent/20 hover:border-accent hover:shadow-lg hover:shadow-green-400/10'
                    : isDemo
              ...
              >
                <div className="flex items-start justify-between mb-4">
                  ...
                  <span className={`
                    ...
                    ${isLive
                      ? 'bg-green-400/10 text-accent border border-accent/20'
                      : isDemo
                      ? 'bg-blue-400/10 text-blue-pill border border-blue-400/20'
                ...
                {isLive && (
                  <div className="mt-4 flex items-center text-accent font-mono text-sm">
                    <span>LAUNCH_TOOL()</span>
                    <span className="ml-2">→</span>
                  </div>
                )}
                {isDemo && (
                  <div className="mt-4 flex items-center text-blue-pill font-mono text-sm">
                    <span>TRY_DEMO()</span>
                    <span className="ml-2">→</span>
                  </div>
                )}
