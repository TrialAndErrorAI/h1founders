...
### Source ingestion
- **WhatsApp**: manual CSV export per group, one-time + quarterly refresh. Quarterly cadence enforced via `_schedule/` reminder, not cron (no API). Each group represented in `metadata_json.whatsapp_groups` array. **WhatsApp export is a Phase 0 exit criterion** (was in Phase 1; promoted because Phase 1 backfill needs the data).
